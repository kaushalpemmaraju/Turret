...
public class RotateTurret extends Command {
  ...
  public void initialize() {
    ...
    Rotation2d pivotAngle =
        new Rotation2d((camera1XAngle.getRadians() + camera2XAngle.getRadians()) / 2.0);
    turretSubsystem.setpivotRotation(pivotAngle);

    Distance distanceToTag =
        Meters.of(
            Math.cos(pivotAngle.getRadians()) * RobotPose.getInstance().getAdaptivePose().getY());

    Distance hubHeight = Meters.of(1.8288);

    Rotation2d rollerAngle =
        Rotation2d.fromRadians(Math.asin(hubHeight.magnitude() / distanceToTag.magnitude()));
