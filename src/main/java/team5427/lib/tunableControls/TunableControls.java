...
public class TunableControls {
  ...
  public static class ControlConstants {
     ...
     *
     * @param profiled whether the controller is profiled. Automatically set to true when {@link
     *     #withProfile(double, double)} is called.
     * @return this ControlConstants object for chaining
     */
    ...
    /**
     * Sets the controller to be continuous over the given input range. Primarily used for angular
     * control. (e.g. 0 and 359 degrees should be considered adjacent)
     *
  ...
  /**
   * Wrapper around a set of control constants that exposes them as {@link LoggedTunableNumber}s for
   * runtime tuning and logging.
   */
  public static class TunableControlConstants {
    ...
    /**
     * Creates a new TunableControlConstants object, initializing the tunable numbers with the
     * values from the given {@link ControlConstants}.
     *
     * @param key base NetworkTables key for the {@link LoggedTunableNumber}s. Should <i>not</i> end
     *     with a slash.
     ...
     *
     * @return array of all LoggedTunableNumbers in the order of: kP, kI, kD, tolerance,
    ...
    /**
     * Creates a new {@link PIDController} (not {@link TunablePIDController}) with the configured
     * constants.
     *
    ...
    /**
     * Creates a new {@link ProfiledPIDController} (not {@link TunableProfiledController}) with the
    ...
    public ProfiledPIDController getProfiledPIDController() {
      ProfiledPIDController controller =
          new ProfiledPIDController(
              ...
              kI.get(),
              kD.get(),
              new TrapezoidProfile.Constraints(maxVel.get(), maxAcc.get()));
      controller.setTolerance(tolerance.get());
      controller.setIntegratorRange(iMin.get(), iMax.get());
      controller.setIZone(iZone.get());
  ...
  /**
   * Wrapper around WPILib's {@link PIDController} that updates the controller with runtime-tunable
   * parameters provided by a {@link TunableControlConstants}.
   *
  ...
  /**
   * Wrapper around WPILib's {@link ProfiledPIDController} and that configures the controller using
   * runtime-tunable values provided by a {@link TunableControlConstants}. It also includes
   * feedforward control (defined by kS, kV, kA, kG).
   *
  ...
  public static class TunableProfiledController {
      ...
      profiledPIDController =
          new ProfiledPIDController(
              ...
              new TrapezoidProfile.Constraints(
                  tunableParams.maxVel.get(), tunableParams.maxAcc.get()),
              tunableParams.period);

      profiledPIDController.setTolerance(
          tunableParams.tolerance.get(), tunableParams.velTolerance.get());
    ...
    /**
     * Updates the controller's parameters based on the current values in the {@link
